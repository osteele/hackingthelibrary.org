...
Note that this commit does three things: it adds the test file, adds the test runner to `requirements.txt`, and it adds the instructions for how to run the tests to the README. The code, the requirements, and the README (and other docs) should all be kept in sync. Consider there to be invariants tThe setup instructions lead to a running system", and "the docs tell me how to use and develop the system". Most changes to the code don't require touching the other two; adding a package or workflow (this change does both) are exceptions.
...
Most languages' test runners have a "watch" option, that re-runs the test when a file changes. This allows you to leave the test running in one pane, and see when your edits break or fix the tests (turn the "red" or "green").

This isn't a feature of pytest itself. Commit #`d3930` therefore also includes [pytest-watch](https://github.com/joeyespo/pytest-watch) among the new package dependencies, and adds usage instructions to the README.
